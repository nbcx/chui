    ...
    public function create($id = 0, $appid = 0) {
        ...
        if ($this->useZh) {
            ...
            for ($i = 0; $i < $this->length; $i++) {
                $tmp= iconv_substr($this->zhSet, floor(mt_rand(0, mb_strlen($this->zhSet, 'utf-8') - 1)), 1, 'utf-8');
                imagettftext($this->im, $this->fontSize, mt_rand(-40, 40), $this->fontSize * ($i + 1) * 1.5, $this->fontSize + mt_rand(10, 20), $this->color, $this->fontttf, $tmp);
                $code .= $tmp;
            }
        }
        else {
            for ($i = 0; $i < $this->length; $i++) {
                $tmp = $this->codeSet[mt_rand(0, strlen($this->codeSet) - 1)];
                $codeNX += mt_rand($this->fontSize * 1.2, $this->fontSize * 1.6);
                imagettftext($this->im, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize * 1.6, $this->color, $this->fontttf, $tmp);
                $code .= $tmp;
            }
        }
