...
class Uploader extends Access {
    ...
    public function base64($base64Data) {
        ...
        $this->ext = '.'.$result[2];
        $this->original  = $this->config['name'].$result[2];
        ...
        $img = base64_decode(str_replace($result[1], '', $base64Data));//base64_decode($base64Data);
        ...
        //移动文件
        if (!(file_put_contents($this->fullPath, $img) && file_exists($this->fullPath))) { //移动失败
